...
export const create = mutation({
    ...
    handler: async (ctx,args)=>
    {
         ...
         if(!receiver)
       {
        ...
        ("receiver", receiver._id).eq("sender",currentUser._id)).unique();
        ...
        .withIndex("by_receiver_sender",(q) => q.eq
        ("receiver", currentUser._id).eq("sender",receiver._id)).unique();
...
export const accepct = mutation({
    ...
    handler: async(ctx, args)=>
    {
        ...
        if(!request || request.receiver !== currentUser._id)
        {
            throw new ConvexError("There was an error accepcting this request");
        }
        ...
        const conversationId = await ctx.db.insert("conversations",{ isGroup: false})
          ...
          await ctx.db.insert("conversationMembers",{memberId: currentUser._id, conversationId})
          await ctx.db.insert("conversationMembers",{memberId: request.sender, conversationId})
          await ctx.db.delete(request._id);
