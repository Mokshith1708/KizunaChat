      ...
      <DialogContent className='block'>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-8' >
                    ...
                    <DropdownMenuContent className='w-full'>
                      {
                                unselectedFriends.map(friend => {
                                  return <DropdownMenuCheckboxItem key={friend._id} className='flex items-center gap-2 w-full p-2' onCheckedChange={checked =>{if(checked){
                                    form.setValue("members",[...members,friend._id])
                                  }}}>

                                    <Avatar className='w-8 h-8'> <AvatarImage src={friend.imageUrl}/>
                                                <AvatarFallback>
              ...
              {
                members && members.length ? <Card className='flex items-centergap-3 overflow-x-auto w-full h-24 p-2 no-scrollbar'>
                  {friends?.filter(friend=> members.includes(friend._id)).map(friend =>{
                    return (<div key={friend._id} className='flex flex-col items-center gap-1'>
                      <div className='relative'>
                            <Avatar><AvatarImage src={friend.imageUrl}/>
                                                <AvatarFallback>
                                                  {friend.username.substring(0,1)}
